...
ctx.drawImage(nlayout, 0, 0)
for (var x = 32; x < 988; x++) {
  ctx.drawImage(nlayout, 32, 430, 1, 300, x, 430, 1, 300)
}
...
btn.forEach(function (button, index) {
  ...
  var dy = index * size
  ctx.drawImage(nlayout, sx, sy, size, size, 0, dy, size, size)
  ctx.drawImage(nlayoutpressed, sx, sy, size, size, size, dy, size, size)
  console.log(
    css(index, '') + ' { background-position: ' + 0 + 'px ' + (-dy) + 'px; }'
  )
  console.log(
    css(index, '[data-state="pressed"]') +
      ' { background-position: ' +
      (-140) +
      'px ' +
      (-dy) +
