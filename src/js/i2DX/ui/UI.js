...
i2DX.ui.UI.prototype = {
  ...
  turntableFullscreen: function (player) {
    ...
    this.add(
      component,
      new i2DX.events.TurntableHandler(component, 'fullscreen')
    )
  },
  ...
  _addTouchHandler: function () {
    document.addEventListener(
      'touchstart',
      i2DX.proxy(this, '_updateTouches'),
      false
    )
    document.addEventListener(
      'touchmove',
  ...
  _updateTouches: function (e) {
    ...
    for (var i = 0; i < touches.length; i++) {
      var touch = this._touches[touches[i].identifier] ||
        (this._touches[touches[i].identifier] = new i2DX.events.Touch(
