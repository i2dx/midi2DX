...
i2DX.events.TurntableHandler.prototype = {
  ...
  _broadcastState: function (stateName) {
    ...
    if (this._lastState != null) {
      i2DX.broadcast(
        'up',
        this._component.getName() + '_' + this._lastState,
        this._component.getPlayer()
      )
    }
  ...
  _getCenter: function () {
    ...
    if (this._placement == 'left') {
      center = {x: window.innerHeight / 2, y: window.innerHeight / 2}
  ...
  _createHandler: function () {
    ...
    return {
      ...
      _extended: false,
      move: function (touch) {
        var angle = Math.atan2(touch.y - center.y, touch.x - center.x) *
          180 /
          Math.PI
